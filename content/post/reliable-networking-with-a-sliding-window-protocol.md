...
To do this, we'll need to talk about some theory. Again, if you want to skip ahead to see the results, the library that I developed can be found [here](https://github.com/rlhunt/netmod).
...
The pseudocode for the protocol's operation is here,
...
1. Receive packets as the come, if the sequence number is LnR then accept it and move LnR forward, otherwise discard the packet. Send an acknowledgment to the transmitter with the new LnR.
...
uint modDifference(uint leadingNumber, uint trailingNumber, uint highestPossibleNumber) {
...
The modified protocol pseudocode is here,
...
You can find the library I created in the process of writing this post here.
